...
**Status in one line: the int8 matmul datapath runs on the real FPGA (64/64 correct);
the full Qwen model does not run on the FPGA yet.** What exists is a bit-exact software
model, every model block written in RTL and verified, the whole 24-layer model run in
simulation predicting the correct next token, and the core int8 matrix-vector engine
built to a real AFI, loaded on a real f2, and returning exact results over PCIe
(`artifacts/fpga_run.txt`). Also: a bandwidth study, a real Vitis synthesis pass, and a
measured HBM bandwidth of 426 GB/s on the chip. What is NOT done: the full model on the
chip (the controller and HBM weight-streaming hardware), and any token generated by the
chip. No prompt has run on the FPGA. Every number below is labeled with where it came
from so nothing reads as more than it is.
