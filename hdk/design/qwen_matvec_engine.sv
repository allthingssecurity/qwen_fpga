...
module qwen_matvec_engine #(
    ...
    wire wr_fire = s_awvalid && s_wvalid && s_awready && s_wready;

    always_ff @(posedge clk) begin
        if (!rst_n) begin
            s_bvalid <= 1'b0; start <= 1'b0; r_in <= 16'd0; r_outrows <= 16'd0;
        end else begin
            start <= 1'b0;
            if (wr_fire) begin
                s_bvalid <= 1'b1;
        ...
        if (!rst_n) begin
            ...
            if (s_arvalid && s_arready) begin
                ...
                unique case (s_araddr[19:16])
                    4'h0: s_rdata <= (s_araddr[7:2]==6'd1) ? {30'd0, busy, done} : 32'd0;
